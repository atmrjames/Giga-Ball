...
| Item | Notes |
|---|---|
| Flipped Angle is too weak | Play-test round 46: the power-up is running - it is collected, timed and shown - but its effect is barely noticeable in play. Needs to be stronger. Note the pairing with the row above: Inert Paddle *does nothing* and Flipped Angle *does too little*, and both are paddle-bounce power-ups whose clocks demonstrably work. Worth looking at them together, because a shared cause in how the bounce angle is composed would explain both |
...
| **Inert Paddle does nothing** | Play-test round 39. **First item of the next session, James's call.** The power-up appears to change no paddle behaviour at all. `endlessIIInertPaddleClock` runs, is collected, shows in the ring HUD and is cleared by a Wipe - so the clock half works and the *effect* is what to find. Start from what reads the clock: `EndlessIIPaddleEffects` for the launch angle it is meant to own, and the bounce path in `paddleHit`, where Sticky-and-Inert together were made to combine (round 22's note is in §12.0). A power-up that is collected, timed and displayed but changes nothing is worse than one that does not drop |
