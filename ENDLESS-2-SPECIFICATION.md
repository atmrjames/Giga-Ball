...
| ~~Endless game-over stats~~ | **Built.** The game-over screen carries the run's numbers in one line - paddle hits, bricks destroyed, power-ups collected - and a stats button (the achievements rosette standing in until §8.5 has one) opens the detail: the headline numbers, then every power-up the run met in the order it met them, each noted collected, missed, falling or active. All read from what the run recorded - the screen computes nothing |
