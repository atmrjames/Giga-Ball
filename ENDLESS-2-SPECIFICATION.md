...
| Sticky Paddle: the ball bounces before it sticks | Play-test round 33: with Sticky Paddle running the ball visibly rebounds a fraction off the paddle and then snaps back onto it. The engine has already resolved the bounce by the time the contact is reported (§8.6's first trap), so the stick is being applied *after* a bounce that has already happened. The fix is to catch it in `update` from `ballStateBeforeStep` - the frame the ball is about to cross the paddle - rather than on contact, the same sample the Portal and the face-detection use |
