...
| Most effective power-up, in the endless modes | Play-test round 46: which power-up bought the most height while it was running. Needs per-power-up accounting the game does not keep - height at collection, height at expiry, the difference accumulated per power-up, plus a best-single-run maximum. That is the array drill again (a 51-slot array in `TotalStats`, its iCloud copy, and the merge rule - highest wins for a best, sum for a total). Design questions to settle first: an *instant* power-up has no window, so it either scores nothing or scores the row it cleared; two power-ups running at once both get credit for the same metres, which is either fine or needs splitting; and Descent gains height by definition, so it will win unless it is excluded or measured against the descent's own rate |
| **Inert Paddle does nothing** - *diagnosed, needs a design decision* | Play-test round 39, investigated round 47. **It is not broken.** The bounce term is `angleAdjustmentK * collisionPercentage * influence`; Inert sets `influence` to 0, which removes the player's ability to steer by where the ball lands. The reason it reads as nothing is that `collisionPercentage` is how far off-centre the catch was, and **near the paddle's middle the whole term is close to zero anyway** - so on a safe, central catch, inert and normal are identical. It is invisible exactly when the player is playing safe, which is most of the time. Flipped Angle had the same cause and could be answered by turning the number up (round 47); Inert cannot, because its number is already zero and there is nothing below it. **So this is a design question for James**, and the options are: (a) leave it - a subtle power-up that punishes only deliberate play is defensible; (b) give it a positive effect rather than an absence, so the paddle actively returns the ball straighter than it arrived, which would be felt on every catch; (c) pair it with something visible, since the paddle dressing is currently the only signal it is on |
...
| Wrecking Ball must destroy Indestructible bricks too | Play-test round 39. **Partly investigated, round 44, not concluded** - and the obvious suspects are cleared, so do not re-check them: `endlessIIWreckingHit` sits *before* the Fixed-brick anchor, the Directional armour guard and the type switch, and it returns straight to `removeBrick`; and `struckBy` really is passed at the ball's contact site, so the `struckBy != nil` condition is met. Whatever stops an Indestructible dying is therefore **upstream of `hitBrick`** - either the contact is not reported for that brick, or something in `didBegin` filters it before the call. Start at `didBegin`'s brick branch rather than in `hitBrick` |
