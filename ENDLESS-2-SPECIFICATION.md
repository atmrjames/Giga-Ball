...
| Average hits per ball, and the best single ball | Play-test round 37: how many paddle hits a ball survives on average, shown on the end-of-game stats and in the main statistics. Average is derivable from what is already stored (`ballHits`/`ballsLost`), so it costs a row in `StatsPage` and one in `RunStatsViewController` and no new storage. The **maximum** hits for a single ball is not derivable and needs a new counter in `TotalStats` plus the per-ball tracking to feed it - which is the array-lengthening drill, and the reason the two halves are not the same size of job |
...
| Pause-menu buttons still animating across each other | Play-test round 37, and **the second report** - round 21's fix (cancelling the mode menu's entry animation with `layer.removeAllAnimations()`) evidently did not cover the in-game pause stack. Moving between the pause menu's own pages animates the play and back buttons over one another. Worth diagnosing rather than patching: find which screen's `showAnimate`/`removeAnimate` is running while the other's is still in flight, because a third partial fix is worse than none |
