...
| **Mazes of indestructible bricks, dotted with normal ones** | Round 190: "Mazes of indestructible bricks dotted with normal bricks so they don't just fly by". A brick sequence rather than a new type: Indestructibles already exist and already survive everything, so this is a generator shape. The dotting is the design point - a pure indestructible maze is a wall the ball rattles off with nothing to earn, and the normal bricks are what make the player want to get in there. Wants a rule for how the maze meets the descent, since an indestructible row that reaches the bottom zone leaves on the same terms as any other row. |
...
| **iPad: the pause and game-over screens lay out in a 420pt box - found, measured, not yet fixed** | Round 188, from the iPad pass. On a 13-inch iPad in portrait - a 1032pt window - the pause screen's whole content block is **422pt wide**, centred. Measured off a native screenshot rather than judged by eye: the Home button's leading edge is at 325pt where the storyboard puts it 20pt inside its container, and every centred item (the title at 396-635pt, "PAUSED" at 427-604pt, the mode name, the play button) is centred on 515-516pt, which is the window's centre. Two consequences a player sees: **Home reads as tucked against the title** instead of sitting in the corner it is meant to sit in (see its own comment - "Home is in the top-left corner *while paused*, where it is out of the way"), and the Information/Play/Settings row bunches into the middle of the screen. On a phone the same screen puts Home in the corner with clear air around it, so this is iPad-only. **What has been ruled out, so nobody repeats it:** it is *not* the round-182 aspect cap - `PauseMenuViewController` never calls `limitMenuContentSize` at all, and nothing else in the app writes `additionalSafeAreaInsets`. It is *not* the `frame`-instead-of-`bounds` presentation bug, which round 188 fixed anyway (below) - the symptom is identical before and after. It is *not* an inherited inset, and this is the decisive evidence: the Information screen *presented from the pause screen* is a child of it and lays out at the full capped 853pt, so the pause menu's own view is full width and it is the storyboard content inside it that is confined. And it is **not the storyboard either** - round 188 brought the measurement into the suite rather than guessing. `MenuResizeTests` now loads `pauseMenuVC` from the storyboard, lays it out at 1032x1376 with no superview, and asserts the box fills the width and Home is within 120pt of the left edge: **both pass**. So the constraints are right and it is the *nesting* the running app adds - the pause menu's view is a subview of the game's, which is a subview of the screen that launched it, and a view's safe area comes from its superview's. The chain is Mode Select (which does apply the cap) to the game view to the pause menu, and the arithmetic does not obviously reach 305 that way, so the next step is to reproduce that nesting in the same test and find where the number comes from. The three tests are the shape any fix has to keep, and one of them holds a phone unchanged. |
