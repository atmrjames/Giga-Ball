...
| Play-test round 88: Auto-Aim, magnetism and a crooked ball | **Auto-Aim never hits the brick it is aiming at**, which makes the whole power-up a lie - the aim marker and the shot disagree, so either `endlessIIWorthAimingAt` picks a target the redirect cannot reach, or the redirect is applied to a heading sampled at the wrong moment (§8.6 again: it must come off `ballStateBeforeStep`, not off the contact). Read those two together before changing either. **It should also skip bricks that do the player no good** - indestructibles, fixed, anything whose hit does nothing - which is what `endlessIIWorthAimingAt` was built for in round 22, so this is that judgement having a gap rather than a missing feature. **The highlighted target should be more prominent.** **Paddle Magnetism reaches too high**: it should start around the second row of bricks from the bottom rather than near the top of the field. **And a ball that changes direction slightly with no brick and no power-up involved** - the most worrying of the five, because it is the physics itself; suspects are the seam-bounce correction (`BrickSeamBounce`), the angle adjustment applied after a paddle hit, and anything nudging velocity from `didSimulatePhysics` on a frame where nothing was struck |
