...
| Paddle surface shapes | New power-up family (play-test idea, fourth round): convex, concave, wavy and jagged paddle tops, all *bad*, all turn-based, each making the outgoing angle harder to predict. One shape function per variant feeding the existing angle line, so they cost a curve rather than a physics body - but they conflict with the paddle group (Inert, Flipped, Auto-Aim at least), which is where the design work is |
...
| Power-up HUD on the pause screen | Play-test round 37: show the in-game ring HUD with the currently active power-ups on the pause screen, and tapping an icon opens a pop-up explaining that power-up. `PowerUpRingHUD` draws the row already and `GigaBallAlert` is the app's own pop-up; the explaining text is `powerUpDescriptionArray`, indexed as everything else in that list is. The pause menu is a view controller and the HUD is SpriteKit, so the row has to be rebuilt in UIKit or snapshotted - deciding which is most of the job |
...
| **A ball was lost and the run continued (Mayhem)** | Play-test round 39, **highest priority of the round**: a ball was lost in Endless Mayhem and the run carried on with another ball. Mayhem has exactly one life; only a Daily twist may change that. Prime suspect is Multi-Ball's bookkeeping - `endlessIIBallsInPlay`, `endlessIIExtraBalls` and `endlessIIHeldBalls` - where a ball removed from one list and not another leaves a phantom that keeps the run alive. Reproduce with Multi-Ball collected, then lose balls one at a time |
| **Force-quit resume rebuilds the field instead of restoring it (all modes)** | Play-test round 39: resuming a Mayhem run gave *different bricks in different places* - not off-row (that was round 37) but a different field entirely, which means the save is being regenerated from a seed rather than restored, or the restore is falling through to generation. Round 37 fixed the row arithmetic and evidently not this. **Check every mode**: the requirement is the run comes back exactly as it was - bricks, ball, paddle, power-ups, clocks |
| Inert Paddle does nothing | Play-test round 39: the power-up appears to change no paddle behaviour at all. `endlessIIInertPaddleClock` runs and is in the wipe list, so the clock works - the effect is what to check |
| Wrecking Ball must destroy Indestructible bricks too | Play-test round 39: it is meant to destroy *every* brick type in one hit, Indestructible included. `endlessIIWreckingHit` is asked before the type switch and returns true, so the likely culprit is an earlier guard - the Indestructible check may sit ahead of it |
| Paddle Halo shrinks with the paddle | Play-test round 39: it should always reach the first two brick rows and stay the same size whatever the paddle's width, rather than scaling with it |
| Auto-Aim rarely hits what it aims at | Play-test round 39. `endlessIIWorthAimingAt` picks a sensible target; the aiming itself sets the velocity straight at the target's centre, which ignores the ball's radius and the brick's face, so near-misses at the edges are expected. Worth checking against a moving target too |
| Restarting Mayhem leaves the old score on screen | Play-test round 39: replaying from game over shows the previous run's height until the first brick is hit. A label not cleared on restart - probably the fixed-width digit strip, which is cleared by `clearPlacedDigits` in some paths and not others |
| Game over: swap home and replay | Play-test round 39, endless and Mayhem: replay becomes the big centre button, home the small one on the left |
| Trajectory Line: two bounces, fading with distance | Play-test rounds 38 and 39. Two brick bounces is the agreed budget. **The line should blur and fade progressively with distance from the ball**, portraying its own declining certainty rather than claiming precision it does not have - and a second collection while it is running sharpens it. That presentation solves the honesty problem `BallPath`'s header raises: a fuzzy far end is not a lie |
...
| Play button still wrong between pause views, and on the backgrounds view | Play-test round 39, **third report**. Round 38 fixed `moveToSettings`; something still animates wrongly between pause pages, and the button is still appearing on the game backgrounds view where `hideReturnToGameButton` was meant to remove it. The re-fronting added in round 36 (`keepReturnToGameButtonFrontmost`, called from `limitMenuContentSize`) may be *undoing* that removal - it re-fronts by tag, and the backgrounds view removes by tag, so whichever runs last wins. Check that first |
| Info button on a swipe-up settings cell still toggles | Play-test round 39, second report: tapping the info button toggles the setting once before showing the pop-up. It should only ever show the pop-up |
| Pop-ups gain an icon above the title | Play-test round 39: an appropriate icon above each pop-up title, in the Giga-Ball green/yellow with the same glow the title wears. `GigaBallAlert` is the type |
