...
| Endless Mayhem badge on the power-ups page | Play-test request, fourth round: the power-ups page should say which entries are Mayhem-only, the way the bricks page says NEW. The mark is the mode's own logo, or a monochrome version of it - a placeholder until §8.5's art lands. The data is already there: `PowerUp.availability == .endlessII` |
...
| Stats table icons | Play-test request, tenth round: small icons left of each row on the stats table view, SF Symbols where possible - the same treatment the run-stats screen's number block already uses (`arrow.up`, `clock`, `rectangle.fill`...). The stats screen is storyboard-built (`StatsViewController`), so this is a cell-layout visit, not a data one |
| Glow around the power-up rings | Play-test request, tenth round: a subtle glow around the ring dials. `PowerUpRingHUD` draws them; SKShapeNode has `glowWidth`, which is the cheap version - worth one visual iteration on the simulator to find a width that reads as glow rather than blur |
| Classic pack-completion score tally | Play-test request, tenth round: finishing a pack should tally the final pack score with the same animated count (and haptic ticking) the endless game-over height gets - `PauseMenuViewController`'s height tally is the pattern to lift (`CADisplayLink`, eased progress, tap to skip). The game-over screen should *always* tally its score, classic included - the request was explicit that every game over animates its number |
