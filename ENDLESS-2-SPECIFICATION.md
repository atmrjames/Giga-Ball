...
Still placeholders: Portal Paddle, Wrap-Around and Cluster (round 178's new power-up). The Daily Challenge badge and the ten
...
| iPad resize audit - **partial pass, round 180** | Play-test round 35: iPadOS 26 resizes every app and ignores `UIRequiresFullScreen`, so multitasking arrived without being adopted. A window-scene minimum of 420x640 is set in `SceneDelegate` so the window can never go below a phone's width - but there is no API to cap the maximum or the ratio, so the audit is real work: every menu screen at split view, slide-over, and the tall-and-thin and short-and-wide extremes, checking `limitMenuContentSize` and the storyboards' constraints hold. The play zone is safe by construction (fixed 1.8236, `GameSceneLayout` asserts it); the menus are the exposure. **Round 180 started it headlessly** (a 13-inch iPad Pro simulator, full screen, portrait): the splash and the main menu both hold - title, credits, mode pills and the two round buttons all placed and readable. **One finding**: the mode pills render as opaque pale cards on the iPad where the iPhone renders them as Liquid Glass, so either the glass path is not taken there or the material reads flat over this backdrop - worth one look at `SettingsTableViewCell.addGlass`'s availability check on the iPad idiom. The rest of the audit (every screen, split view, slide-over, resize extremes) still wants doing with working input - the session's tap channel was down, so screens beyond launch could not be reached. |
