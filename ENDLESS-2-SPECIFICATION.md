...
| Classic force-quit should resume on the between-levels screen | Play-test round 40: quitting on the between-levels view comes back at the *start of the next level*, having skipped it. It should return to the between-levels screen so the player starts the next level themselves. Related to the field-restore bug above and worth doing with it, since both are about a save that restores a *state* rather than a moment |
