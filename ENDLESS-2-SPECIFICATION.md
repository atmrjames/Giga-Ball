...
| **Ball jitter - one cause found, audit open** | Round 200: "the ball still sometimes feels jittery like it's speeding up and slowing down constantly... it seems to happen when certain power ups are enabled. I've noticed it with safety paddle." **Found and fixed with the Safety Paddle**: the pass-through contact was rewriting a climbing ball's velocity through the bounce arithmetic every time the bar's collision bits restored while the ball still edge-touched it. **The audit still owed**: the continuous effects each write velocity per frame - Magnetism turns it, Ball Steering writes position, Ball Spin rotates it, Gravity accelerates it - and any of them interacting with a contact's own correction in the same frame is a speed wobble. The next sighting wants the power-up ring in the screenshot so the suspect list is short. `crookedBallWatch` already logs unexplained bends in DEBUG; a speed-delta tripwire beside it would catch this class directly. |
| **Safety paddle cap rendering** | Round 200: "the rounded edges of the safety paddle don't look the same as the main paddle." It wears the paddle's texture, the shared cap rect and the paddle's own size, so in theory it is identical - suspects: the lime `colorBlendFactor` flattening the texture's shading; the nine-slice interacting with a non-native size; the Retro dress path. Wants a side-by-side screenshot loop, not a blind edit. |
...
| **Two Drifts, one per direction** | Round 200: "one that moves everything left to right and another right to left. Use a mirrored version of the power up icons for the right to left version." A full second power-up slot - the parallel arrays, textures (mirrored art), catalogue, probability, clocks, save keys - with the existing Drift becoming the left-to-right one. Mechanically trivial (`endlessIIDriftDirection` already exists); the work is the slot. Existing saves carrying the old Drift key must restore as left-to-right. |
| **Gravity power-up review** | Round 200: "it works quite strangely because I've tried to prevent the ball moving too fast, too slow, getting stuck, etc... review how the power up works and the ball controls around it, make it more robust, more polished and more predictable." An archaeology-first task: the gravity ball's speed clamps and anti-stuck nudges predate Mayhem and interact with everything added since (the loop-breaker, Randomised Bounce, spin). Wants a written map of every place that touches the ball under gravity before any change - and it reaches Classic and the original Endless, so the leaderboard rule applies: a few levels of play-testing in the old modes before it ships. |
| **Power-up timers ending on a paddle-hit, not at zero** | Round 200: "several classic power-ups like giga-ball, when the timer ends they remain active until the next paddle hit. The idea was to prevent the ball getting stuck inside a brick. Review the logic and come up with something that allows them to end when their timer runs out but prevents any potential issues." The safe shape: end at zero *unless the ball is currently overlapping a brick*, in which case defer to the moment it comes clear - overlap is checkable per frame, and the deferral is then measured in frames rather than in paddle round-trips. Giga-Ball, Undestructi-Ball and ball-size are the affected set. Classic and original Endless carry the leaderboards, so this too wants old-mode play-testing before release. |
| **Power-up bricks gated by kindness, per run** | Round 200: "limited to power-ups that aren't completely disastrous, at least early on... good power ups only, then introduce bad power ups higher up, but not really bad ones, then worse ones even higher. Like everything in Mayhem there should be some randomness to which are available and when." The machinery exists: rounds 192-193's per-run schedule already holds power-ups back by height. What is missing is a *severity tier* on the harmful set (mild / bad / disastrous - the catalogue knows `isHarmful` but not degrees) and the power-up-*brick* draw reading the schedule the way the falling-drop draw already does. Design note: severity should gate the brick draw only - a disastrous power-up can still fall from a brick's destruction early, because a drop can be dodged and a brick's gift cannot. |
