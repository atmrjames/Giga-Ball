...
| Aimed Sticky must freeze *everything* | Play-test bug, fourth round: while a ball is held for aiming, lasers kept firing and the field kept descending - and bricks that moved during the aim ended up below the paddle when the ball was released. §8.6's freeze covers balls, bricks and lasers as *nodes*, but the descent is driven from `update` and the laser cadence from an action, and neither asks whether an aim is in progress. Both must: `launchControl` is the flag, and the descent's timer must resume from where it stopped rather than skipping a step |
| Portal Paddle exit angle | Play-test finding: the ball leaves the top on its old heading, so the paddle-portal gives no control. It should leave the top at the angle a *bounce* off that spot would have given - the same paddle-position-to-angle line the ordinary bounce uses - so where the ball is caught decides where it re-enters. Watch §8.6: the re-entry is written from `didSimulatePhysics`, and the angle must be sampled from the pre-step heading |
| Wrap-Around: the paddle should straddle both walls | Today it snaps across once its centre crosses. A paddle half off one side should appear half on the other - two sprites, or one drawn twice - and its physics body has to follow, which is the real work. Also asked: the side walls wear the portal colours while wrap-around runs (blue left, yellow right), the way the top strip already marks a portal exit |
