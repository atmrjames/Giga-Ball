...
///
/// **Its own setting, not the haptics one.** The eighty-nine places that fire the tap haptic
/// are split between guarding it on `hapticsSetting` and firing it unconditionally, and a
/// sound that inherited either of those would be a sound the player cannot turn off with the
/// control that says Sounds.
enum InterfaceSound {
    ...
    /// Plays it, if the player has sounds on.
