...
extension GameScene {
    ...
    func buildNewEndlessRow(numberOfRows: Int, duration: Double) {

        var brickArray: [SKNode] = []
        // Array to store all bricks
        
        for i in 0..<numberOfRows {
            var randomRow = Int.random(in: 0...99)
            
            if i == numberOfRows-1 && randomRow >= 91 {
                randomRow = 90
            }
            // Stops the row of the same types of bricks being the bottom row
...
//                // Set brick size without stretching edges
        ...
        let waitBrickMove = SKAction.wait(forDuration: duration)
        
        self.run(waitBrickMove, completion: {
        // Keep track of whether a move of the bricks downwards is already in progress
